...
def update_faiss():
    ...
    corpus_embeddings = model.encode_document(corpus, convert_to_tensor=False)
...
def query_docs(query, top_k=3):
    ...
    query_embedding_np = model.encode([query], convert_to_tensor=False)
    faiss_scores, faiss_indices = index.search(query_embedding_np, top_k)
    ...
    for score, idx in zip(faiss_scores[0], faiss_indices[0]):
        ...
        results.append({"score": float(score), "content": corpus[idx]})
